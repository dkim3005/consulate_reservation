...
OPENAI_TTS_INSTRUCTIONS_KO = (
    ...
    "음절을 또박또박 끊어 읽고 문장 끝을 부드럽게 내리며 — 말하세요. "
...
def normalize_caps(name: str) -> str:
    """Display/speech rule: fully-uppercase Latin tokens ("XURI ZHAO") become
    title case ("Xuri Zhao"). TTS reads ALL-CAPS as acronyms, and it reads
    badly on cards too. Mixed-case tokens (McDonald) are left alone."""
    return " ".join(
...
async def _prepare_announcement(name: str, counter: int) -> tuple[str, str, str]:
    ...
    name = name.strip()
    if _has_hangul(name):
        # mixed-script entries ("김시후 Kim Shihu") → speak/display Hangul only
...
QUOTE_PROMPT = (
    "민원 창구에서 하루 종일 사람을 응대하는 직원이 점심시간에 읽고 힘을 얻을 명언을 하나 고르세요. "
    "주제: 사람을 돕는 일의 가치, 친절, 봉사, 묵묵한 수고의 의미. "
    "실제로 널리 알려진 명언이어야 하며 끝에 ' — 인물명'을 표기하세요. "
    "한국어 한 문장, 60자 이내. "
    "이모지, 감탄사, 응원 문구(힘내세요 등), 임의의 서명(동료일동 등)은 절대 넣지 마세요. "
    "명언 문장만 출력하세요."
)
QUOTE_FALLBACKS = [
    "친절은 귀머거리도 들을 수 있고 장님도 볼 수 있는 언어다. — 마크 트웨인",
    "남을 위해 봉사하는 것은 지구에 사는 대가다. — 무하마드 알리",
    "우리가 하는 일은 바다의 물방울 하나지만, 그 한 방울이 없다면 바다는 그만큼 줄어든다. — 테레사 수녀",
    "위대한 일이란 작은 일을 위대한 사랑으로 하는 것이다. — 테레사 수녀",
    "타인을 섬기는 사람만이 진정으로 빛난다. — 알베르트 슈바이처",
]
